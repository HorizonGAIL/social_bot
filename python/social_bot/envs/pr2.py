...
#         steps_since_last_rewards = 0
#         while steps_since_last_rewards < self._max_steps:
#             steps_since_last_rewards += 1
#             goal_loc, _ = goal.get_pose()

#             finger_tip_loc, _ = agent.get_link_pose(
#                 "pr2::pr2::r_gripper_l_finger_tip_link")

#             dist = np.linalg.norm(np.array(finger_tip_loc) - np.array(goal_loc))
#             height_diff = math.fabs(finger_tip_loc[2] - goal_loc[2])
#             if dist < self._success_distance_thresh:
#                 agent_sentence = yield TeacherAction(
#                     reward=10, sentence="well done!", done=True)
#                 steps_since_last_reward = 0
#                 self._move_goal(goal, loc)
#             else:
#                 agent_sentence = yield TeacherAction()
#         yield TeacherAction(reward=(1.2 - height_diff),
#                             sentence="Failed", done=True)
...
class Pr2Gripper(gym.Env):
    ...
    def reset(self):
        ...
        # obs = np.array(self._agent.get_camera_observation("head_mount_prosilica_link_sensor"), copy=False)
        # plt.imshow(obs)
        # plt.pause(0.001)
